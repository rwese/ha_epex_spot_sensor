...
def mock_market_data():
    return [
        {"start_time": dt_util.now().replace(hour=0, minute=0, second=0, microsecond=0) + timedelta(hours=i), 
         "end_time": dt_util.now().replace(hour=0, minute=0, second=0, microsecond=0) + timedelta(hours=i+1), 
         "price": 10.0}
        for i in range(24)
    ]
...
async def test_binary_sensor_setup(hass):
    ...
    with patch("custom_components.epex_spot_sensor.binary_sensor.get_marketdata_from_sensor_attrs", return_value=[]):
